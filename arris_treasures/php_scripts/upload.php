<?php
	...
	// TODO: Update this path to '/images/banner/' when new site is aquired.
	$imageURL = $site_url . "/arris_treasures/images/banner/" . $fileName;
	...
	if($_POST['submitBtn'] == "Insert") {
		...
		$query = "INSERT INTO website_arris_banner(Id, position, image_name, image_url, hyperlink_url, active, date_time) values('', '$position', '$fileName', '$imageURL', '$hyperlink', 1, NOW());";
		$result = @mysql_query($query); // Run the query.
		if($result)
		{
			// Insert successfull.
		...
		if($oldPosition < $position) {
			$query = "select * from website_arris_banner where position > $oldPosition && position <= $position and active = 1 order by position;";
			$result = @mysql_query($query); // Run the query.
			if($result)
			{
				while($row = mysql_fetch_array($result, MYSQL_ASSOC))
				{
					...
					// Now do the update.
					$query2 = "UPDATE website_arris_banner SET position = '$currentSelection' WHERE Id = '$Id';";
					$result2 = @mysql_query($query2); // Run the query.
					if($result2)
					{
						// Update successfull.
			...
			$query = "UPDATE website_arris_banner SET position = '$position', hyperlink_url = '$hyperlink', image_url = '$imageURL', image_name = '$fileName' WHERE Id = '$itemId';";
			$result = @mysql_query($query); // Run the query.
			if($result)
			{
				// Update successfull.
		...
		// Where old position > new position.
		if($oldPosition > $position) {
			$query = "select * from website_arris_banner where position >= $position && position < $oldPosition and active = 1 order by position;";
			$result = @mysql_query($query); // Run the query.
			if($result)
			{
				while($row = mysql_fetch_array($result, MYSQL_ASSOC))
